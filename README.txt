...
The functionality of this plugin is fairly simple. You need to generate Critical Path CSS for your main WordPress pages (e.g. the front page and blog page), coombine and minify the resulting CSS and enter it into the plugin settings. The critical path CSS is inserted inline into the `<head>` of the page and CSS links are loaded asynchronously and rendered via `requestAnimationFrame` API following the [recommendations by Google](https://developers.google.com/speed/docs/insights/OptimizeCSSDelivery).
